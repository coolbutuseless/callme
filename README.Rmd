...
* Straightforward. Requires complete C code - including header includes, and 
  value unpacking.
...
* `callme(code, cpp_flags, ld_flags)` compile and load the function into R.
...
* Switch to using `Makevars` for setting `CPPFLAGS` to be 
  compatible with Windows (which doesn't support `R CMD COMPILE`)
...
* Very little sanity checking.  You must already know how to write C code!
* As required by `.Call()`
    * The C functions must only take `SEXP` arguments, and return a `SEXP` value.
    * All conversion of `SEXP` to C objects must be written explicitly. There's no magic here!
* It's up to the user to track the name of the function in C which is to 
  be used as the first argument to `.Call()`.
* This package tries not to assume anything about your code, or magically 
  take care of object conversion between R and C.
...
The following code snippet asks for the version of the `zstd` library which is
